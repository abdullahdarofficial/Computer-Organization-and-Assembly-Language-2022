...
pcb:	
		...
		dw 0, 0, 0, 0, 0 ; task0 regs[cs:pcb + 30]
		dw 0, 0, 0, 0, 0 ; task3 regs start at [cs:pcb + 40]
		dw 0, 0, 0, 0, 0 ; task4 regs start at [cs:pcb + 50]
...
;---------------------------------------------------------------------------
taskthird:	mov al, [chars+bx]			; read the next shape
...
out 0x40, al
			...
			mov word [pcb+40+4], taskthird			; initialize ip
			mov [pcb+40+6], cs						; initialize cs
			mov word [pcb+40+8], 0x0200				; initialize flags

			mov word [pcb+50+4], taskfourth			; initialize ip
			mov [pcb+50+6], cs						; initialize cs
			mov word [pcb+50+8], 0x0200				; initialize flags
